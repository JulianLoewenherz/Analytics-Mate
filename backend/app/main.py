...
# Annotated video cache (generated by pipeline visualizers)
ANNOTATED_CACHE = Path(__file__).resolve().parent.parent / "annotated_cache"
...
async def get_annotated_video(video_id: str):
    """
    Serve the annotated video (bounding boxes, ROI, color-coding) for a video.
    Generated during pipeline run. Returns 404 if no annotated video exists.
    """
    annotated_path = ANNOTATED_CACHE / f"{video_id}.mp4"
